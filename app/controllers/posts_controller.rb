class PostsController < ApplicationController
  ...
  before_action :find_post, only: %i[edit update destroy update_newsletter]
    ...
    if !(@prev_post_pitch.try(:pitch).nil?)
      if @prev_post_pitch.can_reclaim_pitch?
        ...
        redirect_to @prev_post_pitch.pitch,
                    notice:
                      "Oops you've missed the deadline for this pitch previously."
      end
    else
      fix_formatting
      ...
      if @post.save && @post.is_published?
        redirect_to @post,
                    notice:
                      'Congrats! Your post was successfully published on The Teen Magazine!'
  ...
  def edit
    ...
    @editor_options =
      if (
           (@post.reviews.last.status.eql? 'In Review') ||
  ...
  def update_newsletter
    @newsletter_id =
      if post_params[:newsletter_id].present?
  ...
  def update
    ...
    if @post.update post_params
      ...
      if (
           (@new_status.eql? 'In Review') ||
             ((@new_status.eql? 'Rejected') && (@prev_status.eql? 'In Review'))
         ) && current_user.editor?
      ...
      if @new_status.eql? 'Rejected'
        if @post.deadline_at.present?
          @new_deadline =
            if @post.reviews.last.updated_at > @post.deadline_at
      ...
      if post_params[:promoting_until].present?
        ...
        redirect_to "/writers/#{@post.user.slug}/extensions",
            ...
            post_params[:partner_id].present? &&
              post_params[:partner_id] != false
          )
        @partner = User.find(post_params[:partner_id])
        redirect_to @partner,
                    notice:
                      "This article was successfully shared with #{@partner.full_name}."
      elsif (@new_status.eql? 'In Review') && (current_user.editor?)
        @notice =
          if (@prev_review.editor_id == current_user.id)
