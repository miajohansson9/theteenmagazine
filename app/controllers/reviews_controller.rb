class ReviewsController < ApplicationController
    ...
    if params[:post].present?
      ...
      if !(params[:id].eql? current_user.slug) && !current_user.admin
        redirect_to "/editors/#{current_user.slug}",
                    notice: "You do not have access to that page."
      end
  ...
  def get_editor_activity
    ...
    @editor_reviewed_pitch.each do |pitch|
      Activity.create(
        action: "changed the status of the pitch <a target='_blank' href='/pitches/#{pitch.slug}'>#{pitch.try(:title)}</a> to <b>#{pitch.try(:status)}</b>",
    ...
    @editor_pitched_new_article.each do |pitch|
      Activity.create(
        action: "pitched <a target='_blank' href='/pitches/#{pitch.slug}'>#{pitch.try(:title)}</a>",
    ...
    @writer_claimed_editor_pitch.each do |pitch|
      ...
      Activity.create(
        action: "<span style='margin-left: -5px;'>'s</span> pitch <a target='_blank' href='/pitches/#{pitch.slug}'>#{pitch.try(:title)}</a> was claimed by <a target='_blank' href='/writers/#{@writer.try(:slug)}'>#{@writer.try(:full_name)}</a>",
