class InvitationsController < ApplicationController
  before_action :authenticate_user!, except: %i[show apply_through_invitation]
  skip_before_action :verify_authenticity_token,
                     only: [:apply_through_invitation]
  ...
  def create
    ...
    @active_invite_already_exists =
      current_user
        .invitations
        .where(
          "lower(email) = ? AND status != 'Expired'",
          invitation_params[:email].downcase
        )
        .present?
    @active_user_already_exists =
      User.find_by(email: invitation_params[:email]).present?
    if !(@active_invite_already_exists || @active_user_already_exists)
      if @invitation.save
        ApplicationMailer.invitation_to_apply(
        ...
        redirect_to "/writers/#{current_user.slug}/invitations",
                    notice: 'Oh no, something went wrong!'
      end
    end
  ...
  def show
    ...
    set_meta_tags title:
                    "Invite from #{@invitation.user.first_name} | The Teen Magazine"
  end
