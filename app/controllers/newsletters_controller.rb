class NewslettersController < ApplicationController
  ...
  before_action :find_newsletter, except: [:index, :new]
  ...
  def new
    @newsletter = Newsletter.find_by(kind: nil) || current_user.newsletters.create
    @pagy, @posts = pagy(Post.published.where(newsletter_id: nil), page: params[:page], items: 20)
    @newsletter_posts = []
    @disabled = true
  end
  ...
  def edit
    ...
    @newsletters_to_send_before_cnt = Newsletter.where(sent_at: nil).where("created_at < ?", @newsletter.created_at).count
  ...
  def featured
    ...
    if current_user.admin || current_user.has_newsletter_permissions
      ...
      if params[:search].present?
        ...
        @pagy, @posts = pagy(Post.published.where("lower(title) LIKE ?", "%#{@query.downcase}%"), page: params[:page], items: 15)
      else
        @pagy, @posts = pagy(Post.published.trending, page: params[:page], items: 15)
      end
    else
  ...
  def newsletter_params
    params.require(:newsletter).permit(:message, :kind, :background_color, :sent_at, :ready, :user_id, :hero_image)
  end
