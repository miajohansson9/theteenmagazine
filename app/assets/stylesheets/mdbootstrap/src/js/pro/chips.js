jQuery(($) => {
  ...
  class MaterialChips {
        ...
        typeof options === "string"
          ...
          : {
              ...
              placeholder: this.fallback()
                .or($chipsWrapper.attr("data-placeholder"))
                .or(options.placeholder)
                .or("")
                .value(),
              secondaryPlaceholder: this.fallback()
                .or($chipsWrapper.attr("data-secondary-placeholder"))
                .or(options.secondaryPlaceholder)
                .or("")
                .value(),
              sortAutocompleteData: this.fallback()
                .or($chipsWrapper.attr("data-sort-autocomplete-data"))
                .or(options.sortAutocompleteData)
                .or(true)
                .value(),
              autocompleteDataCompareFn: this.fallback()
                .or(options.autocompleteDataCompareFn)
                .or(undefined)
                .value(),
            };
    ...
    bindChipsWrapperClick() {
      this.$chipsWrapper.on("click", (e) =>
        $(e.target).find("input").focus().addClass("active")
      );
    }
    ...
    bindChipsInputFocusout() {
      this.$chipsWrapper.on("focusout", "input", () =>
        this.$chipsWrapper.removeClass("focus")
      );
    }
    ...
    bindChipsInputKeydown() {
      this.$chipsWrapper.on("keydown", "input", (e) => {
        ...
        if (
          (enterPressed || commaPressed) &&
          !this.$autocompleteList.find("li").hasClass("selected")
        ) {
          e.preventDefault();
        ...
        if (
          isInputEmpty &&
          backspacePressed &&
          !this.$chipsWrapper.find(".chip").hasClass("selected")
        ) {
          const lastChipIndex = this.$chipsWrapper
    ...
    bindAutocompleteInputKeyup() {
      ...
      $input.on("keyup", (e) => {
        ...
        if ($inputValue.length) {
          this.options.dataChip.forEach((item) => {
            if (item.toLowerCase().includes($inputValue.toLowerCase())) {
              this.$chipsWrapper
                .find(".chip-position-wrapper")
                .append(this.$autocompleteList.append($(`<li>${item}</li>`)));
            }
          });
        ...
        const enterPressed = e.which === this.keyCodes.enter;
        const commaPressed = e.which === this.keyCodes.comma;
        const lastChipText = this.$chipsWrapper
          .find(".chip-position-wrapper .chip")
          .last()
          .text();
        ...
        if (
          (enterPressed || commaPressed) &&
          !this.options.dataChip.includes(lastChipText)
        ) {
    ...
    addChip(chip) {
      ...
      if (
        this.$chipsWrapper.hasClass("chips-autocomplete") &&
        this.$chipsWrapper.find(".chip").length > 0
      ) {
    ...
    isValid(chip) {
      return (
        chip.tag !== "" && !this.options.data.some((c) => c.tag === chip.tag)
      );
    }
    ...
    selectChip(chipIndex) {
      ...
      if ($chip && !$chip.hasClass("selected")) {
        ...
        this.$chipsWrapper.trigger(
          "chip.select",
          this.$chipsWrapper.data("chips")[chipIndex]
        );
      }
    }
