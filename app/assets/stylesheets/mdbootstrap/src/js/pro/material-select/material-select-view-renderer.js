export default class MaterialSelectViewRenderer {
  ...
  setMaterialSelectInitialValue() {
    if (!this.view.options.placeholder) {
      const sanitizedLabelHtml = this.view.$materialSelectInitialOption
        .replace(/"/g, "&quot;")
        .replace(/  +/g, " ")
        .trim();
  ...
  enableValidation() {
    ...
    if (
      this.view.$nativeSelect.attr("style").indexOf("inline!important") === -1
    ) {
      this.view.$nativeSelect.attr(
        "style",
        `${this.view.$nativeSelect.attr("style")} display: inline!important;`
      );
    }
  ...
  setMaterialOptionsListMaxHeight() {
    ...
    const $optionsListClone = this.view.$materialOptionsList
      .clone()
    ...
    const optionHeight =
      $materialOptions.first().text().trim() === ""
        ? $materialOptions.eq(1).height()
        : $materialOptions.first().height();
  ...
  appendSearchInputOption() {
    ...
    this.view.$searchInput = $(
      `<span class="search-wrap ml-2"><div class="${divClass} mt-0"><input type="text" class="search w-100 d-block ${inputClass}" tabindex="-1" placeholder="${placeholder}"></div></span>`
    );
    ...
    this.view.$nativeSelectChildren.each((index, option) => {
      ...
      if ($this.is("option")) {
        ...
        const $materialOptgroup = $(
          `<li class="optgroup"><span>${$this.attr("label")}</span></li>`
        );
  ...
  appendToggleAllCheckbox() {
    ...
    if (
      firstOption.hasClass("disabled") &&
      firstOption.find("input").prop("disabled")
    ) {
  ...
  addNewOption() {
    ...
    const $newOption = $(
      `<option value="${val.toLowerCase()}" selected>${val}</option>`
    ).prop("selected", true);
    if (!this.isMultiple) {
      this.view.$nativeSelectChildren.each((index, option) => {
  ...
  buildSingleOption($nativeSelectChild, type) {
    ...
    const fas = $nativeSelectChild.data("fas")
      ? `<i class="fa-pull-right m-2 fas fa-${$nativeSelectChild.data("fas")} ${
          this.view.options.fasClasses
        }"></i> `
      : "";
    const far = $nativeSelectChild.data("far")
      ? `<i class="fa-pull-right m-2 far fa-${$nativeSelectChild.data("far")} ${
          this.view.options.farClasses
        }"></i> `
      : "";
    const fab = $nativeSelectChild.data("fab")
      ? `<i class="fa-pull-right m-2 fab fa-${$nativeSelectChild.data("fab")} ${
          this.view.options.fabClasses
        }"></i> `
      : "";
    ...
    const iconHtml = iconUrl
      ? `<img alt="" src="${iconUrl}" class="${classes}">`
      : "";
    const checkboxHtml = this.isMultiple
      ? `<input type="checkbox" class="form-check-input" ${disabled}/><label></label>`
      : "";
    const secondaryText = $nativeSelectChild.data("secondary-text")
      ? `<p class="text-muted pt-0 mb-0" disabled>${$nativeSelectChild.data(
          "secondary-text"
        )}</p>`
      : "";
    ...
    this.view.$materialOptionsList.append(
      $(
        `<li class="${disabled} ${active} ${optgroupClass}">${iconHtml}<span class="filtrable ${
          this.view.options.copyClassesOption ? classes : ""
        }">${checkboxHtml} ${$nativeSelectChild.html()} ${fas} ${far} ${fab} ${secondaryText}</span></li>`
      )
    );
  }
}
