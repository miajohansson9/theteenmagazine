...
jQuery(($) => {
  class MaterialSelect {
    constructor($nativeSelect, options = {}) {
      this.options = {
        ...
        validate: this.fallback()
          .or($nativeSelect.attr("data-validate"))
          .or(options.validate)
          .or(false)
          .value(),
        selectId: this.fallback()
          .or($nativeSelect.attr("data-select-id"))
          .or(options.selectId)
          .or(null)
          .value(),
        defaultMaterialInput: this.fallback()
          .or($nativeSelect.attr("data-default-material-input"))
          .or(options.defaultMaterialInput)
          .or(false)
          .value(),
        fasClasses: this.fallback()
          .or($nativeSelect.attr("data-fas-classes"))
          .or(options.fasClasses)
          .or("")
          .value(),
        farClasses: this.fallback()
          .or($nativeSelect.attr("data-far-classes"))
          .or(options.farClasses)
          .or("")
          .value(),
        fabClasses: this.fallback()
          .or($nativeSelect.attr("data-fab-classes"))
          .or(options.fabClasses)
          .or("")
          .value(),
        copyClassesOption: this.fallback()
          .or($nativeSelect.attr("data-copy-classes-option"))
          .or(options.copyClassesOption)
          .or(false)
          .value(),
        labels: {
          selectAll: this.fallback()
            .or($nativeSelect.attr("data-label-select-all"))
            .or((options.labels || {}).selectAll)
            .or("Select all")
            .value(),
          optionsSelected: this.fallback()
            .or($nativeSelect.attr("data-label-options-selected"))
            .or((options.labels || {}).optionsSelected)
            .or("options selected")
            .value(),
          validFeedback: this.fallback()
            .or($nativeSelect.attr("data-label-valid-feedback"))
            .or((options.labels || {}).validFeedback)
            .or("Ok")
            .value(),
          invalidFeedback: this.fallback()
            .or($nativeSelect.attr("data-label-invalid-feedback"))
            .or((options.labels || {}).invalidFeedback)
            .or("Incorrect value")
            .value(),
          noSearchResults: this.fallback()
            .or($nativeSelect.attr("data-label-no-search-results"))
            .or((options.labels || {}).noSearchResults)
            .or("No results")
            .value(),
        },
        keyboardActiveClass: this.fallback()
          .or($nativeSelect.attr("data-keyboard-active-class"))
          .or(options.keyboardActiveClass)
          .or("heavy-rain-gradient")
          .value(),
        placeholder: this.fallback()
          .or($nativeSelect.attr("data-placeholder"))
          .or(options.placeholder)
          .or(null)
          .value(),
        visibleOptions: this.fallback()
          .or($nativeSelect.attr("data-visible-options"))
          .or(options.visibleOptions)
          .or(5)
          .value(),
        maxSelectedOptions: this.fallback()
          .or($nativeSelect.attr("data-max-selected-options"))
          .or(options.maxSelectedOptions)
          .or(5)
          .value(),
        showResetButton: this.fallback()
          .or($nativeSelect.attr("data-show-reset-button"))
          .or(options.showResetButton)
          .or(false)
          .value(),
      };

      this.uuid =
        $nativeSelect.attr("id") || this.options.selectId || this._randomUUID();
    ...
    init() {
      ...
      this.view.render();
      this.view.selectPreselectedOptions((optionIndex) =>
        this._toggleSelectedValue(optionIndex)
      );
    ...
    bindEvents() {
      ...
      this.view.bindToggleAllClick((materialOptionIndex) =>
        this._toggleSelectedValue(materialOptionIndex)
      );
      this.view.bindMaterialOptionMousedown();
      this.view.bindMaterialOptionClick((optionIndex) =>
        this._toggleSelectedValue(optionIndex)
      );
      ...
      mutationsList.forEach((mutation) => {
        ...
        if (
          ...
          (mutation.type === "childList" ||
            (mutation.type === "attributes" && $(mutation.target).is("option")))
        ) {
    ...
    _toggleSelectedValue(optionIndex) {
      const selectedValueIndex =
        this.selectedOptionsIndexes.indexOf(optionIndex);
    ...
    _setValueToMaterialSelect() {
      ...
      this.selectedOptionsIndexes.forEach(
        (index) =>
          (value += `, ${this.view.$nativeSelect
            .find("option")
            .eq(index)
            .text()
            .replace(/  +/g, " ")
      ...
      if (
        ...
        selectedValuesCount > this.options.maxSelectedOptions
      ) {
