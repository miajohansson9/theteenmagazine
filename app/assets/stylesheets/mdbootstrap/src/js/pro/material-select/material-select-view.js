...
export default class MaterialSelectView {
  ...
  constructor($nativeSelect, { options, properties: { id } }) {
    ...
    this.$materialSelectInitialOption =
      $nativeSelect.find("option:selected").text() ||
      $nativeSelect.find("option:first").text() ||
      "";
    ...
    this.$materialSelect = $(
      ...
      }" ${!this.options.validate && 'readonly="true"'} required="${
        this.options.validate ? "true" : "false"
      }" ${
        this.$nativeSelect.is(" :disabled") ? "disabled" : ""
      } data-activates="select-options-${this.properties.id}" value=""/>`
    );
    ...
    this.$noSearchResultsInfo = $(
      `<li><span><i>${this.options.labels.noSearchResults}</i></span></li>`
    );
    this.$toggleAll = $(
      `<li class="select-toggle-all"><span><input type="checkbox" class="form-check-input"><label>${this.options.labels.selectAll}</label></span></li>`
    );
    ...
    if (this.isMultiple) {
      this.$nativeSelect
        ...
        .each((i, element) => {
          ...
          if (this._isPlaceholderPresent()) index -= 1;
      ...
      const $preselectedOption = this.$nativeSelect
        .find("option:selected")
        .first();
      const indexOfPreselectedOption = this.$nativeSelect
        .find("option")
        .index($preselectedOption.get(0));
      if (
        $preselectedOption.get(0) &&
        $preselectedOption.attr("disabled") !== "disabled"
      ) {
  ...
  bindMaterialSelectFocus() {
    this.$materialSelect.on("focus", (e) => {
      ...
      if (!this.$materialOptionsList.is(":visible")) {
        ...
        const $selectedOption = this.$materialOptionsList
          .find("li")
          .filter(function () {
  ...
  bindMaterialSelectDropdownToggle() {
    this.$materialSelect.on("open", () =>
      this.$materialSelect.attr("aria-expanded", "true")
    );
    this.$materialSelect.on("close", () =>
      this.$materialSelect.attr("aria-expanded", "false")
    );
  }

  bindToggleAllClick(handler) {
    this.$toggleAll.on("click", (e) => {
      const checkbox = $(this.$toggleAll)
        .find('input[type="checkbox"]')
        .first();
      ...
      this.$materialOptionsList
        ...
        .each((materialOptionIndex, materialOption) => {
          ...
          const $optionCheckbox = $materialOption.find(
            'input[type="checkbox"]'
          );
          ...
          if (
            (isToggleChecked && $optionCheckbox.is(":checked")) ||
            (!isToggleChecked && !$optionCheckbox.is(":checked")) ||
            $(materialOption).is(":hidden") ||
            $(materialOption).is(".disabled")
          ) {
            return;
          }
  ...
  bindMaterialOptionMousedown() {
    this.$materialOptionsList.on("mousedown", (e) => {
      ...
      const inModal = $(".modal-content").find(
        this.$materialOptionsList
      ).length;
      ...
      .each((materialOptionIndex, materialOption) => {
        $(materialOption).on("click", (e) => {
          ...
          if (this.isMultiple) {
            $this
              .find('input[type="checkbox"]')
              .prop("checked", (index, oldPropertyValue) => !oldPropertyValue);

            const hasOptgroup = Boolean(
              this.$nativeSelect.find("optgroup").length
            );
            const thisIndex =
              this._isToggleAllPresent() && !this._isPlaceholderPresent()
                ? $this.index() - 1
                : $this.index();
            ...
            this.$materialSelect.val(
              $($selectedOption).text().replace(/  +/g, " ").trim()
            );
            this.$materialSelect.trigger("close");
          }
          ...
          $this.attr(
            "aria-selected",
            ariaSelected === "true" ? "false" : "true"
          );
          this._selectSingleOption($this);
          this.$nativeSelect.data("stop-refresh", true);
          const selectedOptionIndex = this._isPlaceholderPresent()
  ...
  bindSearchInputKeyup() {
    this.$searchInput.find(".search").on("keyup", (e) => {
      ...
      this.$addOptionBtn[
        searchValue && this.isEditable && !isOptionInList ? "show" : "hide"
      ]();

      const anyOptionMatch =
        $options.filter(
          (_, e) => $(e).is(":visible") && !$(e).parent().hasClass("disabled")
        ).length !== 0;
      if (!anyOptionMatch) {
        this.$toggleAll.hide();
  ...
  bindHtmlClick() {
    $("html").on("click", (e) => {
      if (
        !$(e.target).closest(`#select-options-${this.properties.id}`).length &&
        !$(e.target).hasClass("mdb-select") &&
        $(`#select-options-${this.properties.id}`).hasClass("active")
      ) {
        this.$materialSelect.trigger("close");
  ...
  bindMobileDevicesMousedown() {
    $("select")
      ...
      .on("mousedown", (e) => {
        if (
          MaterialSelectView.isMobileDevice &&
  ...
  _updateToggleAllOption() {
    const $allOptionsButToggleAll = this.$materialOptionsList
      .find("li")
      .not(".select-toggle-all, .disabled, :hidden")
      .find("[type=checkbox]");
    const $checkedOptionsButToggleAll =
      $allOptionsButToggleAll.filter(":checked");
    const isToggleAllChecked = this.$toggleAll
      .find("[type=checkbox]")
      .is(":checked");

    if (
      $checkedOptionsButToggleAll.length === $allOptionsButToggleAll.length &&
      !isToggleAllChecked
    ) {
  ...
  _handleEnterKey($materialSelect) {
    const $activeOption = this.$materialOptionsList.find(
  ...
  _handleArrowUpDownKey(keyCode) {
    // eslint-disable-next-line object-curly-newline
    const { $matchedMaterialOption, $activeOption } =
      this._getArrowMatchedActiveOptions(keyCode, false);
    ...
    if (
      !$activeOption.hasClass("selected") &&
      !$activeOption.find("input").is(":checked") &&
      this.isMultiple
    ) {
    ...
    if ($matchedMaterialOption.position()) {
      this.$materialOptionsList.scrollTop(
        this.$materialOptionsList.scrollTop() +
          $matchedMaterialOption.position().top
      );
    }
  }
  ...
  _handleClosedArrowUpDownKey(keyCode) {
    // eslint-disable-next-line object-curly-newline
    const { $matchedMaterialOption } = this._getArrowMatchedActiveOptions(
      keyCode,
      true
    );
  ...
  _getArrowMatchedActiveOptions(keyCode, closedDropdown) {
    ...
    const $availableOptions = this.$materialOptionsList
      .find(`li${visible}`)
    ...
    if (isArrowUp) {
      const $currentOption = anySelected
        ? this.$materialOptionsList.find("li.selected").first()
        : $lastOption;
      let $prevOption = $currentOption.prev(
      ...
      const $currentOption = anySelected
        ? this.$materialOptionsList.find("li.selected").first()
        : $firstOption;
      let $nextOption = $currentOption.next(
        `li${visible}:not(.disabled, .select-toggle-all)`
      );
  ...
  _selectBoundaryOption(firstOrLast = "") {
    const $boundaryOption = this.$materialOptionsList
      .find("li:visible")
      .not(".disabled, .select-toggle-all")
      [firstOrLast]();
    ...
    if ($boundaryOption.position()) {
      this.$materialOptionsList.scrollTop(
        this.$materialOptionsList.scrollTop() + $boundaryOption.position().top
      );
    }
  }
  ...
  _handleLetterKey(e) {
    ...
    if (this.isSearchable) {
      ...
      const nonLetters = Object.keys(this.keyCodes).map(
      ...
      if (isLetterSearchable) {
        ...
        const $matchedMaterialOption = this.$materialOptionsList
          .find("li")
          .filter((index, element) =>
            $(element).text().toLowerCase().includes(filterQueryString)
          )
          .first();
  ...
  _updateDropdownScrollTop() {
    const $preselected = this.$materialOptionsList
      .find("li.active")
      .not(".disabled")
      .first();
