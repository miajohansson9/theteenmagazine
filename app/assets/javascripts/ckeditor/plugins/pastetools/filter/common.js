...
(function () {
  ...
  function r(a) {
    var b = a.margin ? "margin" : a.MARGIN ? "MARGIN" : !1,
      ...
      e;
    if (b) {
      e = CKEDITOR.tools.style.parse.margin(a[b]);
      for (c in e) a["margin-" + c] = e[c];
      delete a[b];
    }
  }
  function t(a) {
    ...
    for (e in c)
      (f = e + ":" + c[e]),
        CKEDITOR.tools.array.some(b, function (a) {
          return f.substring(0, a.length).toLowerCase() === a;
        }) && delete c[e];
    ...
    return (
      CKEDITOR.tools.array.find(b, function (a) {
        for (var f = 0; f < c.length; f++)
          if (-1 === a.indexOf(CKEDITOR.tools.trim(c[f]))) return !1;
        return !0;
      }) || a
    );
  }
      ...
      elements: {
        ...
        table: function (a) {
          ...
          if (
            ...
            a.remove();
            for (k = e.length - 1; 0 <= k; k--) d.add(e[k], b.getIndex());
            b.remove();
          }
          h.convertStyleToPx(a);
        },
        tr: function (a) {
          a.attributes = {};
        },
        td: function (a) {
          var g = a.getAscendant("table"),
            g = l.parseCssText(g.attributes.style, !0),
            d = g.background;
          d && h.setStyle(a, "background", d, !0);
          (g = g["background-color"]) &&
            h.setStyle(a, "background-color", g, !0);
          var g = l.parseCssText(a.attributes.style, !0),
            d = g.border
              ? CKEDITOR.tools.style.border.fromCssRule(g.border)
              : {},
            d = l.style.border.splitCssValues(g, d),
            e = CKEDITOR.tools.clone(g),
            k;
          for (k in e) 0 == k.indexOf("border") && delete e[k];
          a.attributes.style = CKEDITOR.tools.writeCssText(e);
          g.background &&
            ((k = CKEDITOR.tools.style.parse.background(g.background)),
            k.color &&
              (h.setStyle(a, "background-color", k.color, !0),
              h.setStyle(a, "background", "")));
          for (var m in d)
            (k = g[m] ? CKEDITOR.tools.style.border.fromCssRule(g[m]) : d[m]),
              "none" === k.style
                ? h.setStyle(a, m, "none")
                : h.setStyle(a, m, k.toString());
          h.mapCommonStyles(a);
          h.convertStyleToPx(a);
          h.createStyleStack(
            a,
        ...
        font: function (a) {
          a.attributes.face &&
            e &&
            (a.attributes.face = v(a.attributes.face, e));
        },
      },
    };
  };
  p.styles = {
    setStyle: function (a, b, c, e) {
      var f = l.parseCssText(a.attributes.style);
      (e && f[b]) ||
        ("" === c ? delete f[b] : (f[b] = c),
    ...
    mapStyles: function (a, b) {
      for (var c in b)
        if (a.attributes[c]) {
          ...
          else h.setStyle(a, b[c], a.attributes[c]);
    ...
    mapCommonStyles: function (a) {
      return h.mapStyles(a, {
        vAlign: function (b) {
          h.setStyle(a, "vertical-align", b);
        },
        width: function (b) {
          h.setStyle(a, "width", q(b));
        },
        height: function (b) {
          h.setStyle(a, "height", q(b));
        },
      });
    },
    normalizedStyles: function (a, b) {
      ...
      "cke:li" == a.name &&
        (d["TEXT-INDENT"] && d.MARGIN
          ? ((a.attributes["cke-indentation"] =
              p.lists.getElementIndentation(a)),
            (d.MARGIN = d.MARGIN.replace(
              /(([\w\.]+ ){3,3})[\d\.]+(\w+$)/,
              "$10$3"
            )))
          : delete d["TEXT-INDENT"],
        delete d["text-indent"]);
      for (var n = l.object.keys(d), k = 0; k < n.length; k++) {
        var m = n[k].toLowerCase(),
          h = d[n[k]],
          q = CKEDITOR.tools.indexOf;
        ((g && -1 !== q(e, m.toLowerCase())) ||
          f(null, m, h) ||
          f(null, m.replace(/\-.*$/, "-")) ||
          f(null, m) ||
          f(a.name, m, h) ||
      ...
      var t = CKEDITOR.plugins.pastetools.getConfigValue(b, "keepZeroMargins");
      r(d);
      (function () {
        CKEDITOR.tools.array.forEach(
          ...
          function (a) {
            a = "margin-" + a;
            if (a in d) {
              var b = CKEDITOR.tools.convertToPx(d[a]);
              b || t ? (d[a] = b ? b + "px" : 0) : delete d[a];
            }
          }
        );
      })();
      return CKEDITOR.tools.writeCssText(d);
    },
    createStyleStack: function (a, b, c, e) {
      ...
      h.sortStyles(a);
      b = l.parseCssText(h.normalizedStyles(a, c));
      c = a;
      var g = "span" === a.name,
        d;
      for (d in b)
    ...
    sortStyles: function (a) {
      for (
        var b = ["border", "border-bottom", "font-size", "background"],
          c = l.parseCssText(a.attributes.style),
          e = l.object.keys(c),
          f = [],
          g = [],
          d = 0;
        ...
        -1 !== l.indexOf(b, e[d].toLowerCase()) ? f.push(e[d]) : g.push(e[d]);
      f.sort(function (a, c) {
        var d = l.indexOf(b, a.toLowerCase()),
          f = l.indexOf(b, c.toLowerCase());
        return d - f;
      });
      e = [].concat(f, g);
      f = {};
      for (d = 0; d < e.length; d++) f[e[d]] = c[e[d]];
      a.attributes.style = CKEDITOR.tools.writeCssText(f);
    },
    pushStylesLower: function (a, b, c) {
      ...
      var e = {
        ...
        f = l.parseCssText(a.attributes.style),
        g;
      for (g in f)
        if (
          !(
            g.toLowerCase() in e ||
            e[g.toLowerCase().replace(/\-.*$/, "-")] ||
            g.toLowerCase() in b
          )
        ) {
          for (var d = !1, n = 0; n < a.children.length; n++) {
            var k = a.children[n];
            if (k.type === CKEDITOR.NODE_TEXT && c) {
    ...
    inliner: {
      ...
      parse: function (a) {
        ...
        function c(a) {
          var b = a.indexOf("{"),
            c = a.indexOf("}");
          return e(a.substring(b + 1, c), !0);
        }
        var e = CKEDITOR.tools.parseCssText,
          f = h.inliner.filter,
          g = a.is ? a.$.sheet : b(a);
        a = [];
        var d;
        if (g)
          for (g = g.cssRules, d = 0; d < g.length; d++)
            g[d].type === window.CSSRule.STYLE_RULE &&
              a.push({
                selector: g[d].selectorText,
                styles: f(c(g[d].cssText)),
              });
        return a;
      },
      filter: function (a) {
        var b = h.inliner.filtered,
          c = l.array.indexOf,
          e = {},
          f;
        for (f in a) -1 === c(b, f) && (e[f] = a[f]);
        return e;
      },
      sort: function (a) {
        return a.sort(
  ...
  p.createAttributeStack = function (a, b) {
    ...
    for (c = a.children.length - 1; 0 <= c; c--)
      e.unshift(a.children[c]), a.children[c].remove();
    c = a.attributes;
    var f = a,
      g = !0,
      d;
    ...
    for (c = 0; c < e.length; c++) f.add(e[c]);
  };
