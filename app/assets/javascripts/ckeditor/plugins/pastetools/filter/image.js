...
(function () {
  function f(b) {
    var c = [],
      ...
      d;
    b = b.match(new RegExp("(?:(" + a.source + "))([\\da-fA-F\\s]+)\\}", "g"));
    if (!b) return c;
    for (var e = 0; e < b.length; e++)
      if (a.test(b[e])) {
        ...
        c.push({
          hex: d ? b[e].replace(a, "").replace(/[^\da-fA-F]/g, "") : null,
          type: d,
        });
      }
    return c;
  ...
  function g(b) {
    for (var c = /<img[^>]+src="([^"]+)[^>]+/g, a = [], d; (d = c.exec(b)); )
      a.push(d[1]);
    return a;
  }
  CKEDITOR.pasteFilters.image = function (b, c, a) {
    ...
    if (c.length === d.length)
      for (a = 0; a < c.length; a++)
        0 === c[a].indexOf("file://") && d[a] && (b = b.replace(c[a], d[a]));
    return b;
  };
})();
