...
(function () {
  ...
  function p(a, c) {
    ...
    (function (b) {
      function c(a, b) {
        ...
        if (d) {
          var f = (d[1][0] || "").toLowerCase() + d[1].substr(1);
      ...
      a.on("beforeCommandExec", function (e) {
        c(e.data.name, b) &&
          e.data.command.style.checkActive(e.editor.elementPath(), a) &&
          (e.cancel(), a.focus());
      });
      for (var d in b) f(a, d, b[d], "easyimage" + g(d));
    })(c);
  }
  function q(a) {
  ...
  function t(a, c) {
    var b = a.config,
      ...
      b = {
        ...
        upcasts: {
          figure: function (a) {
            if ((!f || a.hasClass(f)) && 1 === a.find("img", !0).length)
              return a;
          },
        },
        init: function () {
          function b(a, c) {
            var d = a.$;
            if (d.complete && d.naturalWidth) return c(d.naturalWidth);
            a.once("load", function () {
              if (!d.naturalWidth) return (d.src = d.src), b(a, c);
              c(d.naturalWidth);
            });
          }
          var c = this.parts.image,
            d = c && c.$.complete && !c.$.naturalWidth;
          if ((c && !c.$.complete) || d)
            d && (c.$.src = c.$.src),
              b(c, function () {
        ...
        _loadDefaultStyle: function () {
          var b = !1,
            f = a.config.easyimage_defaultStyle,
            d;
          for (d in c) {
            var e = a.getCommand("easyimage" + g(d));
            !b &&
              e &&
              e.style &&
              -1 !== CKEDITOR.tools.array.indexOf(e.style.group, "easyimage") &&
              this.checkStyleActive(e.style) &&
              (b = !0);
          }
          !b &&
            f &&
            a.getCommand("easyimage" + g(f)) &&
            this.applyStyle(a.getCommand("easyimage" + g(f)).style);
        },
      };
    f &&
      ((b.requiredContent += "(!" + f + ")"),
      (b.allowedContent.figure.classes =
        "!" + f + "," + b.allowedContent.figure.classes));
  ...
  function u(a) {
    var c = new RegExp(
      "\x3cimg[^\x3e]*\\ssrc\x3d[\\'\\\"]?data:image/(" +
        l.join("|") +
        ");base64,",
      "i"
    );
    a.on("paste", function (b) {
      if (!a.isReadOnly && c.test(b.data.dataValue)) {
        ...
        var f = document.implementation.createHTMLDocument(""),
          ...
          g = a.widgets.registered.easyimage,
          l = 0,
          d,
          e,
          h,
        ...
        for (m = 0; m < e.count(); m++) {
          h = e.getItem(m);
          var k = (d = h.getAttribute("src")) && "data:" == d.substring(0, 5),
            n = null === h.data("cke-realelement");
          k &&
            n &&
            !h.isReadOnly(1) &&
            (l++,
            1 < l &&
              ...
              k[0].collapse(!1)),
            d.match(/image\/([a-z]+?);/i),
            (k = g._spawnLoader(a, d, g)),
            (d = g._insertWidget(a, g, d, !1, { uploadId: k.id })),
            d.data("cke-upload-id", k.id),
            d.replace(h));
        }
