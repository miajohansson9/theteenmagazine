...
(function () {
  ...
  var useFlex = !CKEDITOR.env.ie || CKEDITOR.env.edge,
    templateSource =
      '<div class="cke_reset_all cke_dialog_container {editorId} {editorDialogClass} {hidpi}' +
      '" dir="{langDir}"' +
      ' style="' +
      ...
      ' cke_{langDir}"' +
      ' style="' +
      (useFlex ? "margin:auto" : "position:absolute") +
      '"' +
      ' role="presentation">' +
  ...
  function buildDialog(editor) {
    var element = CKEDITOR.dom.element.createFromHtml(
      CKEDITOR.addTemplate("dialog", templateSource).output({
        id: CKEDITOR.tools.getNextNumber(),
        editorId: editor.id,
        langDir: editor.lang.dir,
        langCode: editor.langCode,
        editorDialogClass:
          "cke_editor_" + editor.name.replace(/\./g, "\\.") + "_dialog",
  ...
  CKEDITOR.dialog = function (editor, dialogName) {
    ...
    if (
      (buttonsOrder == "OS" && CKEDITOR.env.mac) || // The buttons in MacOS Apps are in reverse order (https://dev.ckeditor.com/ticket/4750)
    ...
    this.definition = definition = CKEDITOR.fire(
      "dialogDefinition",
      {
        name: dialogName,
    ...
    if (
      editor._.removeDialogTabs &&
      (tabsToRemove = editor._.removeDialogTabs[dialogName])
    ) {
      ...
      function (evt) {
        iterContents(function (item) {
          if (item.validate) {
            ...
            handleFieldValidated.call(
              item,
              !invalid,
              typeof retval == "string" ? retval : undefined
            );
            return invalid;
          }
        });
    ...
    this.on(
      ...
      function (evt) {
        iterContents(function (item) {
          if (item.isChanged()) {
            if (
              !editor.config.dialog_noConfirmCancel &&
              !confirm(editor.lang.common.confirmCancel)
            )
              // jshint ignore:line
              evt.data.hide = false;
            return true;
    ...
    function setupFocus() {
      ...
      focusList.sort(function (a, b) {
        ...
        if (a.tabIndex != b.tabIndex) return b.tabIndex - a.tabIndex;
      ...
      for (var i = 0; i < size; i++) focusList[i].focusIndex = i;
    }
    ...
    function changeFocus(offset) {
      ...
      do {
        ...
        if (
          hasTabs &&
          !me._.tabBarMode &&
          (currentIndex == focusList.length || currentIndex == -1)
        ) {
    ...
    function keydownHandler(evt) {
      ...
      if (me != CKEDITOR.dialog._.currentTop) return;
      ...
      if (keystroke == 9 || keystroke == CKEDITOR.SHIFT + 9) {
        ...
        CKEDITOR.tools.indexOf(arrowKeys, keystroke) != -1 &&
        me._.tabBarMode
      ) {
        ...
        var prevKeyCodes = [
          ...
          nextId =
            CKEDITOR.tools.indexOf(prevKeyCodes, keystroke) != -1
              ? getPreviousVisibleTab.call(me)
              : getNextVisibleTab.call(me);
      ...
      // If user presses enter key in a text box, it implies clicking OK for the dialog.
        ...
        if (
          !target.is("a", "button", "select", "textarea") &&
          (!target.is("input") || target.$.type != "button")
        ) {
    ...
    function keypressHandler(evt) {
      if (processed) evt.data.preventDefault(1);
    ...
    this.on("iframeAdded", function (evt) {
      var doc = new CKEDITOR.dom.document(
        evt.data.iframe.$.contentWindow.document
      );
    ...
    // Insert the title.
    new CKEDITOR.dom.text(definition.title, CKEDITOR.document).appendTo(
      this.parts.title
    );
    ...
    // Insert buttons.
    var buttonsHtml = [],
      buttons = CKEDITOR.dialog._.uiElementBuilders.hbox
        .build(
          this,
          {
            type: "hbox",
  ...
  function Focusable(dialog, element, index) {
    ...
    element.on("keydown", function (e) {
      if (e.data.getKeystroke() in { 32: 1, 13: 1 }) this.fire("click");
    });
  ...
  CKEDITOR.dialog.prototype = {
    ...
    resize: function (width, height) {
      if (
        this._.contentSize &&
        this._.contentSize.width == width &&
        this._.contentSize.height == height
      )
        return;
      ...
      if (this._.editor.lang.dir == "rtl" && this._.position) {
        var containerWidth = this.parts.dialog
          .getParent()
          .getClientSize().width;
    ...
    show: function () {
      ...
      if (CKEDITOR.dialog._.currentTop === null) {
        ...
        this._.parentDialog
          .getElement()
          .setStyle("z-index", parentElement.$.style.zIndex);
        CKEDITOR.dialog._.currentTop = this;
      }
      ...
      for (var i in definition.contents) {
        ...
        for (var j in this._.contents[content.id]) {
          ...
          if (
            elem.type == "hbox" ||
            elem.type == "vbox" ||
            !elem.getInputElement()
          ) {
            continue;
          }

          if (
            elem.requiredContent &&
            !this._.editor.activeFilter.check(elem.requiredContent)
          ) {
            elem.disable();
          } else {
            elem.enable();
        ...
        if (
          !enableElements ||
          (requiredContent &&
            !this._.editor.activeFilter.check(requiredContent))
        ) {
    ...
    setupContent: function () {
      ...
      this.foreach(function (widget) {
        if (widget.setup) widget.setup.apply(widget, args);
      });
    },
    ...
    hide: function () {
      ...
      if (!this._.parentDialog) {
        ...
        parentElement.setStyle(
          "z-index",
          parseInt(parentElement.$.style.zIndex, 10) +
    ...
    addPage: function (contents) {
      if (
        contents.requiredContent &&
        !this._.editor.filter.check(contents.requiredContent)
      ) {
        return;
      }

      var pageHtml = [],
        titleHtml = contents.label
          ? ' title="' + CKEDITOR.tools.htmlEncode(contents.label) + '"'
          : "",
        vbox = CKEDITOR.dialog._.uiElementBuilders.vbox.build(
          this,
          {
            type: "vbox",
    ...
    selectPage: function (id) {
      if (this._.currentTabId == id) return;

      if (this._.tabs[id][0].hasClass("cke_dialog_tab_disabled")) return;
    ...
    updateStyle: function () {
      ...
      this.parts.dialog[(this._.pageCount === 1 ? "add" : "remove") + "Class"](
        "cke_single_page"
      );
    },
    ...
    setState: function (state) {
      ...
      if (state == CKEDITOR.DIALOG_STATE_BUSY) {
        ...
        if (!this.parts.spinner) {
          ...
          spinnerDef.styles["margin-" + (dir == "rtl" ? "left" : "right")] =
            "8px";

          this.parts.spinner = CKEDITOR.document.createElement(
            "div",
            spinnerDef
          );
    ...
    getMode: function (editor) {
      ...
      if (
        !model ||
        (model instanceof CKEDITOR.dom.element && !model.getParent())
      ) {
  ...
  CKEDITOR.tools.extend(CKEDITOR.dialog, {
    ...
    isTabEnabled: function (editor, dialogName, tabName) {
      ...
      return !(
        cfg &&
        cfg.match(
          new RegExp("(?:^|;)" + dialogName + ":" + tabName + "(?:$|;)", "i")
        )
      );
    },
  ...
  var removeById = function (array, id, recurse) {
    for (var i = 0, item; (item = array[i]); i++) {
      if (item.id == id) return array.splice(i, 1);
      if (recurse && item[recurse]) {
  ...
  function initDragAndDrop(dialog) {
    ...
    function mouseMoveHandler(evt) {
      ...
      if (abstractDialogCoords.x + margins[3] < magnetDistance) {
        ...
        abstractDialogCoords.x - margins[1] >
        containerSize.width - dialogSize.width - magnetDistance
      ) {
        realX =
          containerSize.width -
          dialogSize.width +
          (editor.lang.dir == "rtl" ? 0 : margins[1]);
      } else {
      ...
      if (abstractDialogCoords.y + margins[0] < magnetDistance) {
        ...
        abstractDialogCoords.y - margins[2] >
        containerSize.height - dialogSize.height - magnetDistance
      ) {
    ...
    var mouseDownFn = CKEDITOR.tools.addFunction(function ($event) {
      ...
      if (iframeDialog) {
        dialogCover = CKEDITOR.dom.element.createFromHtml(
          '<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%; left:0; top:0;"></div>'
        );
      ...
      wrapperHeight =
        ...
        dialog.parts.contents
          .getFirst(isVisible)
          .getSize("height", isBorderBox);
      wrapperWidth =
        startSize.width -
        dialog.parts.contents.getFirst(isVisible).getSize("width", 1);
    ...
    dialog.on("load", function () {
      ...
      var resizer = CKEDITOR.dom.element.createFromHtml(
        "<div" +
          ...
          '"' +
          ' title="' +
          CKEDITOR.tools.htmlEncode(editor.lang.common.resize) +
          '"' +
  ...
  function showCover(editor) {
    var config = editor.config,
      ...
      backgroundColorStyle =
        ...
        (skinName == "moono-lisa" ? "black" : "white"),
    ...
    if (!coverElement) {
      ...
      coverElement.setOpacity(
        backgroundCoverOpacity !== undefined ? backgroundCoverOpacity : 0.5
      );
  ...
  function removeCovers() {
    for (var coverId in covers) covers[coverId].remove();
    covers = {};
  }
  ...
  function accessKeyDownHandler(evt) {
    ...
    keyProcessor.keydown &&
      keyProcessor.keydown.call(
        keyProcessor.uiElement,
        keyProcessor.dialog,
        keyProcessor.key
      );
    evt.data.preventDefault();
  ...
  (function () {
    CKEDITOR.ui.dialog = {
      ...
      uiElement: function (
        ...
        var nodeName =
            (nodeNameArg.call ? nodeNameArg(elementDefinition) : nodeNameArg) ||
            "div",
          html = ["<", nodeName, " "],
          styles =
            (stylesArg && stylesArg.call
              ? stylesArg(elementDefinition)
              : stylesArg) || {},
          attributes =
            (attributesArg && attributesArg.call
              ? attributesArg(elementDefinition)
              : attributesArg) || {},
          innerHTML =
            (contentsArg && contentsArg.call
              ? contentsArg.call(this, dialog, elementDefinition)
              : contentsArg) || "",
          domId = (this.domId =
            attributes.id || CKEDITOR.tools.getNextId() + "_uiElement"),
          i;

        if (
          elementDefinition.requiredContent &&
          !dialog
            .getParentEditor()
            .filter.check(elementDefinition.requiredContent)
        ) {
          styles.display = "none";
        ...
        for (i = 0; i < attributeClasses.length; i++) {
          if (attributeClasses[i]) classes[attributeClasses[i]] = 1;
        }
        var finalClasses = [];
        ...
        for (i = styleStr.length - 1; i >= 0; i--) {
          if (styleStr[i] === "") styleStr.splice(i, 1);
        }
        ...
        if (
          this.accessKeyUp &&
          this.accessKeyDown &&
          elementDefinition.accessKey
        )
          registerAccessKey(
            this,
            dialog,
            "CTRL+" + elementDefinition.accessKey
          );
        ...
        var innerHTML = function () {
          ...
          for (i = 0; i < childHtmlList.length; i++) {
            ...
            if (widths) {
              ...
              styles.push(
                "width:" + Math.floor(100 / childHtmlList.length) + "%"
              );
            }
            if (height) {
        ...
        var innerHTML = function () {
          ...
          html.push(
            ...
            CKEDITOR.tools.htmlEncode(
              (elementDefinition && elementDefinition.align) ||
                (dialog.getParentEditor().lang.dir == "ltr" ? "left" : "right")
            ),
            '" '
          );
          ...
          for (var i = 0; i < childHtmlList.length; i++) {
            ...
            if (heights) styles.push("height:" + cssLength(heights[i]));
              ...
              styles.push(
                "height:" + Math.floor(100 / childHtmlList.length) + "%"
              );
            if (elementDefinition && elementDefinition.padding !== undefined)
              styles.push("padding:" + cssLength(elementDefinition.padding));
            // In IE Quirks alignment has to be done on table cells. (https://dev.ckeditor.com/ticket/7324)
  ...
  CKEDITOR.ui.dialog.uiElement.prototype = {
    ...
    selectParentTab: function () {
      ...
      while (
        ...
        cursor.$.className.search("cke_dialog_page_contents") == -1
      ) {}
    ...
    registerEvents: function (definition) {
      ...
      for (var i in definition) {
        if (!(match = i.match(regex))) continue;
      ...
      getChild: function (indices) {
        ...
        if (indices.length < 2) return this._.children[indices[0]];
          ...
          return this._.children[indices[0]] &&
            this._.children[indices[0]].getChild
            ? this._.children[indices[0]].getChild(
    ...
    var commonBuilder = {
      build: function (dialog, elementDefinition, output) {
        ...
        for (var i = 0; i < children.length && (child = children[i]); i++) {
          ...
          childObjList.push(
            CKEDITOR.dialog._.uiElementBuilders[child.type].build(
              dialog,
              child,
              childHtml
            )
  ...
  CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
    ...
    openDialog: function (dialogName, callback, forceModel) {
      var dialog = null,
        dialogDefinitions = CKEDITOR.dialog._.dialogDefinitions[dialogName];
      ...
      if (typeof dialogDefinitions == "function") {
        ...
        CKEDITOR.scriptLoader.load(
          CKEDITOR.getUrl(dialogDefinitions),
          function () {
            var dialogDefinition =
              CKEDITOR.dialog._.dialogDefinitions[dialogName];
  ...
  CKEDITOR.plugins.add("dialog", {
    ...
    init: function (editor) {
      ...
      editor.on(
        "doubleclick",
        function (evt) {
          if (evt.data.dialog) editor.openDialog(evt.data.dialog);
        },
        null,
        null,
        999
